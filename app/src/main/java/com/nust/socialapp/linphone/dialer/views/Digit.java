 ...
 */
package com.nust.socialapp.linphone.dialer.views;
...
public class Digit extends Button implements AddressAware {
    ...
    public Digit(android.content.Context context, AttributeSet attrs, int style) {
        super(context, attrs, style);
        init(context, attrs);
    }

    public Digit(android.content.Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }

    public Digit(android.content.Context context) {
        super(context);
        setLongClickable(true);
    }

    private void init(android.content.Context context, AttributeSet attrs) {
    ...
    private class DialKeyListener implements OnClickListener, OnTouchListener, OnLongClickListener {
        ...
        public boolean onTouch(View v, MotionEvent event) {
            ...
            Manager.getCallManager().resetCallControlsHidingTimer();

            Core core = Manager.getCore();
            if (event.getAction() == MotionEvent.ACTION_DOWN && !mIsDtmfStarted) {
                Manager.getCallManager().playDtmf(getContext().getContentResolver(), mKeyCode);
                mIsDtmfStarted = true;
            } else {
        ...
        public boolean onLongClick(View v) {
            ...
            if (id == R.id.Digit1 && core.getCalls().length == 0) {
                ...
                if (voiceMail != null) {
                    ...
                    Manager.getCallManager().newOutgoingCall(mAddress);
                }
                return true;
            }
