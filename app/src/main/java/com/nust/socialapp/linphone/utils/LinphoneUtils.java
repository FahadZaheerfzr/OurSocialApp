 ...
 */
package com.nust.socialapp.linphone.utils;
...
public final class LinphoneUtils {
        ...
        if (!LinphonePreferences.instance().useJavaLogger()) {
            ...
            if (isDebugEnabled) {
                if (Context.isReady()) {
                    Factory.instance()
                            .getLoggingService()
                            .addListener(Context.instance().getJavaLoggingService());
                }
            } else {
                if (Context.isReady()) {
                    Factory.instance()
                            .getLoggingService()
                            .removeListener(Context.instance().getJavaLoggingService());
                }
            }
        }
    }
    ...
    public static boolean isNumberAddress(String numberOrAddress) {
        ProxyConfig proxy = Manager.getCore().createProxyConfig();
    ...
    public static String timestampToHumanDate(
            android.content.Context context, long timestamp, int format) {
        return timestampToHumanDate(context, timestamp, context.getString(format));
    }

    public static String timestampToHumanDate(
            android.content.Context context, long timestamp, String format) {
        try {
    ...
    public static boolean isHighBandwidthConnection(android.content.Context context) {
    ...
    public static void reloadVideoDevices() {
        ...
        Manager.getInstance().resetCameraFromPreferences();
    }
    ...
    public static void displayErrorAlert(String msg, android.content.Context ctxt) {
    ...
    public static void showTrustDeniedDialog(android.content.Context context) {
        ...
        message.setText("Trust has been denied");
        ...
        call.setOnClickListener(
                new View.OnClickListener() {
                    ...
                    public void onClick(View view) {
                        CallLog[] logs = Manager.getCore().getCallLogs();
                        CallLog lastLog = logs[0];
                        Address addressToCall =
                                lastLog.getDir() == Call.Dir.Incoming
                                        ? lastLog.getFromAddress()
                                        : lastLog.getToAddress();
                        Manager.getCallManager().newOutgoingCall(addressToCall.asString(), null);
                        dialog.dismiss();
                    }
                });
        dialog.show();
    ...
    public static Dialog getDialog(android.content.Context context, String text) {
        final Dialog dialog = new Dialog(context);
        dialog.requestWindowFeature(Window.FEATURE_NO_TITLE);
        Drawable d = new ColorDrawable(ContextCompat.getColor(context, R.color.dark_grey_color));
        d.setAlpha(200);
